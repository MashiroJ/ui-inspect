...
function installProjectPackage(project: string, packageName: string): boolean {
...
function nextIntegrationSteps(
  router: NextRouterKind,
  integration: NextIntegrationInspection,
  missing: string[] = integration.missing,
): string[] {
  ...
  if (missing.includes(INTEGRATION_PACKAGES.next)) steps.push(install);
  if (router === 'app') {
