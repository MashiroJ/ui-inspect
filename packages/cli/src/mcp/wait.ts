...
export async function waitForFrontendRequest(
  ...
  while (Date.now() <= deadline) {
    ...
    const request = latestFrontendRequest(
      sessionId ? { sessions: sessions.sessions.filter((session) => session.id === sessionId) } : sessions,
      sinceTimestamp,
      afterRequestId,
    );
...
export function extractResponseMode(args: ToolArgs): 'compact' | 'full' {
...
function normalizeResultTargets(session: UiInspectSession): UiInspectTarget[] {
...
async function readSourceSafely(selection: UiInspectSelection, context: number): Promise<UiInspectSourceResponse | { error: string } | null> {
  if (!selection.source?.root || !selection.source?.file) return null;
  try {
...
function compactSelection(selection: UiInspectSelection): Record<string, unknown> {
  return {
    id: selection.id,
...
function summarizeRequestContext(selection: UiInspectSelection | null, message: UiInspectMessage): string {
  const parts: string[] = [];
  if (selection) {
  ...
  if (request) parts.push(`Request: ${request.slice(0, 160)}`);
  return parts.length ? `Element: ${parts.join(' • ')}` : '';
}

function summarizeRequestTargets(targets: UiInspectTarget[]): string {
  if (!targets.length) return 'No targets.';
  return targets.map((target, index) => {
    const selection = target.selection;
    const title = clean(
      selection.context?.accessibleName ||
...
function summarizeElementSnapshot(selection: UiInspectSelection | null): string {
  if (!selection?.dom) return 'No selected element.';
...
function buildDomSearchHints(selection: UiInspectSelection | null, targets: UiInspectTarget[]): DomSearchHint[] {
  const seen = new Set<string>();
  const hints: DomSearchHint[] = [];
  const add = (token: string, reason: string, priority: DomSearchHint['priority']) => {
    const normalized = normalizeSearchToken(token);
    if (!normalized || seen.has(normalized) || isLowValueDomToken(normalized)) return;
    seen.add(normalized);
    hints.push({
      token: normalized,
      reason,
  ...
  ].filter(Boolean) as UiInspectSelection[];
  ...
  for (const item of selections) {
    ...
    if (item.dom?.id) add(item.dom.id, 'selected element id', 'high');
...
function elementLabel(selection: UiInspectSelection): string {
  const dom = selection.dom;
  const attrs: string[] = [];
  if (dom.id) attrs.push(`id="${dom.id}"`);
  if (dom.className) attrs.push(`class="${clean(dom.className).slice(0, 160)}"`);
  return `<${dom.tagName || 'element'}${attrs.length ? ` ${attrs.join(' ')}` : ''}>`;
}

function attributesFromDom(selection: UiInspectSelection): Record<string, string> {
  const attrs: Record<string, string> = {};
  if (selection.dom?.id) attrs.id = selection.dom.id;
  if (selection.dom?.className) attrs.class = selection.dom.className;
  return attrs;
}

function styleSnapshot(styles: Record<string, string>): string[] {
...
function classesFromSelection(selection: UiInspectSelection): string[] {
  return splitClassName(selection.dom?.className);
}

function classesFromSummary(summary: { selector?: string; attributes?: Record<string, string> }): string[] {
...
function splitClassName(className: string | undefined): string[] {
...
function isLowValueDomToken(token: string): boolean {
  const value = token.toLowerCase();
  if (value.length < 3 || value.length > 120) return true;
  if (/^\d+$/.test(value)) return true;
  if (/^(div|span|main|body|html|cell|panel|layout|container|content|wrapper|inner|item|row|col|left|right|top|bottom|active|disabled)$/.test(value)) return true;
  if (/^(el-|ant-|van-|ivu-|arco-|n-|q-|v-|zvue-|cursor-|data-v-)/.test(value)) return true;
  return false;
}
...
function summarizeSourceHints(selection: UiInspectSelection | null, targets: UiInspectTarget[]): string {
  const hints = [
...
function summarizeRuntimeDiagnostics(diagnostics: UiInspectDiagnostics | undefined): string {
  ...
  if (diagnostics?.truncated) lines.push('Diagnostics were truncated in the browser.');
  return lines.join('\n');
}
